#!/bin/bash
#SBATCH --job-name=mlp_few_shot          # Job name
...
for target in "${TARGET_VARIABLES[@]}"; do
    ...
    python train_few_shot.py data.target_variable=$target data.z_min=0 data.z_max=2 training.devices=-1 logging.experiment_tag=mlp
done
...
for pair in "${VALID_PAIRS[@]}"; do
    ...
    python train_few_shot.py data.target_variable=$target data.z_min=$z_min data.z_max=$z_max training.devices=-1 logging.experiment_tag=mlp
done